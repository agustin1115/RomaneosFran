...
def calcular_score(reporte, rend_objetivo=66.0):
    ...
    if diff_rend >= 3:
        ...
        fundamentos_positivos.append(f"Rendimiento excelente ({rend:.1f}%, +{diff_rend:.1f}% sobre objetivo)")
    elif diff_rend >= 1.5:
        scores['rendimiento'] = 8
        fundamentos_positivos.append(f"Buen rendimiento ({rend:.1f}%, +{diff_rend:.1f}% sobre objetivo)")
    elif diff_rend >= 0:
        scores['rendimiento'] = 6
    elif diff_rend >= -2:
        scores['rendimiento'] = 4
        fundamentos_negativos.append(f"Rendimiento bajo objetivo ({rend:.1f}%, {diff_rend:.1f}%)")
    else:
        scores['rendimiento'] = max(1, 2 + diff_rend)
        fundamentos_negativos.append(f"Rendimiento muy bajo ({rend:.1f}%, {diff_rend:.1f}% vs objetivo {rend_objetivo:.0f}%)")

    # 2. % Cortes caros (25%)
    pct_caros = reporte['pct_caros']
    if pct_caros >= 42:
        scores['cortes_caros'] = 10
        fundamentos_positivos.append(f"Excelente proporcion de cortes caros ({pct_caros:.0f}%)")
    elif pct_caros >= 38:
        scores['cortes_caros'] = 8
        fundamentos_positivos.append(f"Buena proporcion de cortes caros ({pct_caros:.0f}%)")
    elif pct_caros >= 34:
        scores['cortes_caros'] = 6
    elif pct_caros >= 30:
        scores['cortes_caros'] = 4
        fundamentos_negativos.append(f"Pocos cortes caros ({pct_caros:.0f}%) - mas kg fueron a cortes baratos")
    else:
        scores['cortes_caros'] = 2
        fundamentos_negativos.append(f"Muy pocos cortes caros ({pct_caros:.0f}%) - la mayoria fue a picada/recorte")
    ...
    if pct_am <= 2:
        ...
        scores['amarilla'] = 9
        fundamentos_positivos.append(f"Amarilla muy baja ({pct_am:.0f}%)")
    elif pct_am <= 10:
        scores['amarilla'] = 7
    elif pct_am <= 20:
        scores['amarilla'] = 5
        fundamentos_negativos.append(f"Amarilla moderada ({pct_am:.0f}%) - reduce el precio de venta promedio")
    elif pct_am <= 35:
        scores['amarilla'] = 3
        fundamentos_negativos.append(f"Mucha amarilla ({pct_am:.0f}%) - castiga fuerte el ingreso")
    else:
        scores['amarilla'] = 1
        fundamentos_negativos.append(f"Amarilla muy alta ({pct_am:.0f}%) - gran parte se vende a precio minimo")

    # 4. Carne picada (10%) — inverso: menos = mejor
    pct_picada = reporte['pct_picada']
    if pct_picada <= 12:
        scores['picada'] = 10
        fundamentos_positivos.append(f"Poca picada ({pct_picada:.0f}%) - los kg fueron a cortes de valor")
    elif pct_picada <= 16:
        scores['picada'] = 7
    elif pct_picada <= 20:
        scores['picada'] = 5
        ...
        scores['picada'] = 3
        fundamentos_negativos.append(f"Mucha picada ({pct_picada:.0f}%) - exceso de recortes")
    else:
        scores['picada'] = 1
        fundamentos_negativos.append(f"Picada excesiva ({pct_picada:.0f}%) - demasiado va al corte mas barato")
    ...
    if precio <= 6200:
        scores['precio'] = 10
        fundamentos_positivos.append(f"Precio de compra muy competitivo (${precio:,.0f}/kg)")
    elif precio <= 6600:
        scores['precio'] = 8
        fundamentos_positivos.append(f"Buen precio de compra (${precio:,.0f}/kg)")
    elif precio <= 7000:
        scores['precio'] = 6
    elif precio <= 7400:
        scores['precio'] = 4
        fundamentos_negativos.append(f"Precio de compra alto (${precio:,.0f}/kg)")
    else:
        scores['precio'] = 2
        fundamentos_negativos.append(f"Precio de compra muy alto (${precio:,.0f}/kg) - ajusta el margen")
...
def generar_pdf_comprador(reportes_con_score, fecha_reporte=None):
    ...
    for idx, r in enumerate(sorted_rep):
        ...
        vals = [
            ...
            str(rep['medias']),
            f"{rep['rend']:.1f}",
            f"{rep['pct_amarilla']:.0f}",
            f"{rep['pct_caros']:.0f}",
            f"${rep['precio_compra']:,.0f}",
            f"{puntaje:.1f}",
        ]
    ...
    pdf.cell(0, 8, f"Mejor compra: {mejor['reporte']['nombre']} ({mejor['score']['puntaje']}/10)",
    ...
    pdf.cell(0, 8, f"Peor compra: {peor['reporte']['nombre']} ({peor['score']['puntaje']}/10)",
             new_x='LMARGIN', new_y='NEXT')
    pdf.set_font('Helvetica', '', 9)
    pdf.set_text_color(0, 0, 0)
    ...
    for r in sorted_rep:
        ...
        pdf.cell(40, 25, f'{puntaje}', align='C')
        pdf.set_font('Helvetica', 'B', 14)
        pdf.cell(20, 25, f'/ 10')
        ...
        datos = [
            ...
            ('Kg entrada', f"{rep['kg_entrada']:,.0f}"),
            ('Kg carne', f"{rep['kg_carne']:,.0f}"),
            ('Rendimiento', f"{rep['rend']:.1f}%"),
            ('Precio compra', f"${rep['precio_compra']:,.0f}/kg"),
            ('Amarilla', f"{rep['pct_amarilla']:.1f}%"),
            ('Cortes caros', f"{rep['pct_caros']:.1f}%"),
            ('Carne picada', f"{rep['pct_picada']:.1f}%"),
        ]
        ...
        for nombre, peso, nota in factores:
            ...
            pdf.cell(25, 6, f'{pond:.1f}', border=1, align='C', fill=True)
            pdf.ln()
