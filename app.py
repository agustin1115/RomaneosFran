...
tab_upload, tab_analisis, tab_control, tab_comprador, tab_pricing, tab_revoleo, tab_export, tab_historial = st.tabs([
    "📤 Cargar archivos",
    "📊 Análisis",
    "🔍 Control de cortes",
    "🛒 Reporte Comprador",
...
with tab_upload:
    ...
    if st.session_state.parsed_files:
        ...
        nombres_all = [p.get('archivo', '?') for p in valid_all]
        excluir = st.multiselect(
            "❌ Excluir archivos (seleccioná los que NO querés)",
            nombres_all, key='excluir_archivos'
        )
        if excluir:
            for p in st.session_state.parsed_files:
                if p.get('archivo') in excluir:
                    p['excluido'] = True
                else:
                    p.pop('excluido', None)
        ...
        if excluir:
            st.caption(f"Mostrando {len(valid_parsed)} de {len(valid_all)} ({len(excluir)} excluidos)")
